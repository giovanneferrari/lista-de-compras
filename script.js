...
const listItemInput = document.querySelectorAll('.item-check')
const alertEl = document.querySelectorAll('.alert')
const buttonEl = document.querySelector('button')
const listContainer = document.querySelector('section div')

listContainer.addEventListener ("click", function(event) {
    if (event.target.classList.contains('bin')) {
        event.target.parentNode.remove()
        alertEl[0].style.display = 'flex'
    }
})

alertEl.forEach (close => {
...
buttonEl.addEventListener('click', (event) => {
    ...
    item.innerHTML = `<div class="item-list-info">
                        <input type="checkbox" name="item" class="item-check">
                        <p>${inputEl.value}</p>
                    </div>
                    <svg width="16" height="17" viewBox="0 0 16 17" fill="none" xmlns="http://www.w3.org/2000/svg" class="bin">
    ...
    itemDelete = document.querySelectorAll('.bin')
})
